(function ($) {
	$(document).ready(function () {
		...
		form.addEventListener("submit", function (e) {
			...
			// Alte Einträge (>24h) entfernen
			submissions = submissions.filter(timestamp => now - timestamp < TIME_WINDOW_MS);

			if (submissions.length >= MAX_SUBMISSIONS) {
				e.preventDefault();
				alert("Du hast das Limit von 3 Anfragen in 24 Stunden erreicht.");
				return;
			}
		...
		const hour = new Date().getHours(); // Holen der aktuellen Stunde
		let imageUrl;


		// Bild basierend auf der Uhrzeit
		if (hour >= 7 && hour < 19) {
			// Morgen: 07:00–19:00 Uhr
			imageUrl = 'images/Banner_Mittags.jpg';  // Bild für Morgen
		} else if (hour >= 19 && hour < 23) {
			// Mittag: 19:00–23:00 Uhr
			imageUrl = 'images/Banner_Abends.jpg';  // Bild für Mittag
		} else {
			// Nacht: 23:00–07:00 Uhr
			imageUrl = 'images/Banner_Nachts.jpg';  // Bild für Nacht
		}

		// Finden des .banner-image div und setze das Hintergrundbild
		const bannerImageDiv = document.querySelector('.banner-image');
		if (bannerImageDiv) {
			bannerImageDiv.style.backgroundImage = `url('${imageUrl}')`;
			bannerImageDiv.style.backgroundSize = 'cover';    // Bildgröße anpassen
			bannerImageDiv.style.backgroundPosition = 'center'; // Bild zentrieren
		}
	...
	// Dynamischer Textwechsel & Sichtbarkeitssteuerung für Mobile/Desktop
	document.addEventListener("DOMContentLoaded", function () {
		...
		const heroHeadline = document.querySelector(".hero-caption h2");
		const heroParagraph = document.querySelector(".hero-caption p");
		...
		function updateBannerText() {
			...
			if (window.innerWidth < 768) {
				// Mobile-Variante
				bannerText.innerText = "Ihr Auto. Unser Anspruch.";
				if (sublineText) {
					sublineText.innerText = "Persönlich & modern";
				}
				...
				if (heroHeadline) {
					heroHeadline.innerText = "Herzlich willkommen!";
				}

			} else {
				// Desktop-Variante
				bannerText.innerText = "Autoservice auf höchstem Niveau – wir setzen Maßstäbe";
				if (sublineText) {
					sublineText.innerText = "Kompetenz trifft Leidenschaft.";
				}
				if (heroParagraph) {
					heroParagraph.style.display = "block";
				}
				if (heroHeadline) {
					heroHeadline.innerText = "Ihr Partner für modernen Autoservice – herzlich willkommen!";
				}
			}
		}

		updateBannerText(); // Beim Laden
		window.addEventListener("resize", updateBannerText); // Bei Fensteränderung
	});
